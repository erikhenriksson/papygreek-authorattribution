...
class EmbeddingTransformer(BaseEstimator, TransformerMixin):
    ...
    def embed(self, w, weights):
        vectors = np.asarray(
            [
                self.model[x] * (weights.get(x, 1)) if x in w else [0.0]
                for x in w.split()
            ]
        )
        if vectors.size:
...
def ngram_transformers(name, n1, n2):
    analyzers = {
        "character_ngrams_bow": (
            f"character_nrams_bow-{n1}/{n2}",
            CountVectorizer(analyzer="char", ngram_range=(n1, n2)),
            "text",
        ),
        "character_ngrams_tfidf": (
            f"character_nrams_tfidf-{n1}/{n2}",
...
def get_ngram_transformers(func_n):
    parts = func_n.split("-")
    func_name = parts[0]
    ns = parts[1]
    if "/" in ns:
        ...
        n1 = int(ns)
        n2 = int(ns)
    return ngram_transformers(func_name, n1, n2)
...
def transformers(init_model=None):
    ...
    return [
        ...
        # Fasttext
        ("words_fasttext", EmbeddingTransformer(model), "text"),
        ("words_fasttext_tfidf", EmbeddingTransformer(model, tfidf=True), "text"),
        ("function_words_fasttext", EmbeddingTransformer(model), "function_words"),
        (
...
def get_stats(target, df, tail):
    ...
    print(
        f"{target.capitalize()}s with most texts{tail_text}: \n--------------------\n{person_texts}\n"
    )

    person_tokens = (
        df.groupby("person")["text_len"]
        .sum()
        .sort_values()
        .tail(tail)
        .to_string(header=False)
    ...
    print(
        f"{target.capitalize()}s with most tokens{tail_text}: \n--------------------\n{person_tokens}\n"
    )

    person_variations = (
        df.groupby("person")["variations_len"]
        .sum()
        .sort_values()
        .tail(tail)
        .to_string(header=False)
    ...
    print(
        f"{target.capitalize()}s with most variations{tail_text}: \n--------------------\n{person_variations}\n"
    )

    text_len = (
        df.sort_values(by="text_len")
        .tail(tail)[["person", "text_id", "text_len"]]
        .to_string(index=False)
    )
    print(
        f"{target.capitalize()} texts with most tokens{tail_text}: \n--------------------\n{text_len}\n"
    )

    text_variations = (
        df.sort_values(by="variations_len")
        .tail(tail)[["person", "text_id", "variations_len"]]
        .to_string(index=False)
    )
    print(
        f"{target.capitalize()} texts with most variations{tail_text}: \n--------------------\n{text_variations}\n"
    )
...
def get_column_transformer(features):
    ...
    transformer_functions = [
        (tr[0], tr[1], tr[2]) for tr in transformers(model) if tr[0] in features
    ]

    ngram_functions = [
        get_ngram_transformers(feat) for feat in features if "ngram" in feat
    ]

    if ngram_functions:
        transformer_functions += ngram_functions
    print(transformer_functions)
    return ColumnTransformer(transformer_functions)
...
def run(args, _):
    ...
    target_files = {"writer": "writer_texts.csv", "author": "author_texts.csv"}
    ...
    # Sort
    df = df.sort_values(
        by="person", key=lambda x: np.argsort(index_natsorted(df["person"]))
    )
    ...
    for k, v in vars(args).items():
        if v:
            if k == "persons":
                ...
                df = df[
                    df["variations"].apply(lambda x: len(x.split()) >= args.min_vars)
                ]
            elif k == "min_tokens":
    ...
    if args.mode == "classification":
        ...
        disp = ConfusionMatrixDisplay(
            confusion_matrix=cm, display_labels=unique_persons
        )
        ...
        # Get n clusters
        n_clusters = (
            len(list(set([x.replace("?", "") for x in unique_persons])))
        ...
        # Get kmeans clusters
        kmeans = KMeans(
            n_clusters=n_clusters, max_iter=500, n_init=10, random_state=0
        ).fit(X)
        ...
        for _, row in df2.iterrows():
            ...
            texts.append(
                ax.text(
                    *ax_text, size="smaller" if target_dimensions == 3 else "medium"
                )
            )
            ...
            if row["person"] not in seen_persons:
                ...
                handles.append(
                    mpatches.Patch(
                        color=colors[row["person_unique"]], label=row["person"]
                    )
                )
        ...
        if target_dimensions == 2:
            ...
            for i, _ in enumerate(zs_l):
                ax.plot(
                    xs=[df2.x[i]] * 2, ys=[df2.y[i]] * 2, zs=zs_l[i], color="lightgrey"
                )
